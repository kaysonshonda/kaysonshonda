...
$validationRules = [
    ...
    'serviceType' => ['required' => true, 'type' => 'string', 'minLength' => 2, 'maxLength' => 100],
...
$serviceType  = sanitizeInput($req_body['serviceType'] ?? '', 'string');
...
// 🔹 Load Email Template
$templatePath = __DIR__ . '/templates/vehicle-service.html';
...
$body = str_replace(
    ['{{fullName}}', '{{model}}', '{{email}}', '{{mobileNumber}}', '{{serviceType}}', '{{message}}'],
    [$fullName, $model, $email, $mobileNumber, $serviceType, nl2br($message)],
    $template
);
...
try {
    ...
    $mail->setFrom('checkbox.webd@gmail.com', 'Kaysons Vehicle Service Form');
    $mail->addAddress('checkbox.webd@gmail.com');

    // Email Content
    $mail->isHTML(true);
    $mail->Subject = 'Kaysons Vehicle Service Form';
    $mail->Body    = $body;
