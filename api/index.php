...
$validationRules = [
    ...
    'phone' => ['required' => true, 'type' => 'phone', 'pattern' => '/^[\+]?[0-9\s\-()]{10,20}$/'],
    'note' => ['required' => false, 'type' => 'string', 'maxLength' => 5000],
];
...
$phone        = sanitizeInput($req_body['phone'] ?? '', 'phone');
$note         = sanitizeInput($req_body['note'] ?? '', 'string');

// 🔹 Load Email Template
$templatePath = __DIR__ . '/templates/booking-email.html';
...
$body = str_replace(
    ['{{fullName}}', '{{model}}', '{{email}}', '{{phone}}', '{{note}}'],
    [$fullName, $model, $email, $phone, nl2br($note)],
    $template
);
...
try {
    ...
    $mail->setFrom('checkbox.webd@gmail.com', 'Kaysons Contact Form');
    $mail->addAddress('checkbox.webd@gmail.com');

    // Email Content
    $mail->isHTML(true);
    $mail->Subject = 'Kaysons Contact Form';
    $mail->Body    = $body;
