...
$validationRules = [
    ...
    'partName' => ['required' => true, 'type' => 'string', 'minLength' => 2, 'maxLength' => 100],
...
$partName     = sanitizeInput($req_body['partName'] ?? '', 'string');
...
// 🔹 Load Email Template
$templatePath = __DIR__ . '/templates/spare-parts.html';
...
$body = str_replace(
    ['{{fullName}}', '{{model}}', '{{email}}', '{{mobileNumber}}', '{{partName}}', '{{message}}'],
    [$fullName, $model, $email, $mobileNumber, $partName, nl2br($message)],
    $template
);
...
try {
    ...
    $mail->setFrom('checkbox.webd@gmail.com', 'Kaysons Spare Parts Request');
    $mail->addAddress('checkbox.webd@gmail.com');

    // Email Content
    $mail->isHTML(true);
    $mail->Subject = 'Kaysons Spare Parts Request';
    $mail->Body    = $body;
