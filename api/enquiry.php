...
// 🔹 Load Email Template
$templatePath = __DIR__ . '/templates/enquiry.html';
...
$body = str_replace(
    ['{{fullName}}', '{{model}}', '{{email}}', '{{mobileNumber}}', '{{message}}'],
    [$fullName, $model, $email, $mobileNumber, nl2br($message)],
    $template
);
...
try {
    ...
    $mail->setFrom('checkbox.webd@gmail.com', 'Kaysons Enquiry Form');
    $mail->addAddress('checkbox.webd@gmail.com');

    // Email Content
    $mail->isHTML(true);
    $mail->Subject = 'Kaysons Enquiry Form';
    $mail->Body    = $body;
