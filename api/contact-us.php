...
$validationRules = [
    ...
    'subject' => ['required' => true, 'type' => 'string', 'minLength' => 2, 'maxLength' => 100],
...
// 🔹 Sanitize Valid Data
$name     = sanitizeInput($req_body['name'] ?? '', 'string');
$subject        = sanitizeInput($req_body['subject'] ?? '', 'string');
$email        = sanitizeInput($req_body['email'] ?? '', 'email');
$message         = sanitizeInput($req_body['message'] ?? '', 'string');

// 🔹 Load Email Template
$templatePath = __DIR__ . '/templates/contact-email.html';
...
$body = str_replace(
    ...
    [$name, $subject, $email, nl2br($message)],
    $template
);
